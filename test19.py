...
def rand_beneficiary():
    ...
    if r < 0.25:
        ...
        # "a.k jacob" / "A.K. Jacob" like patterns
        style = random.randint(0, 2)
        if style == 0:
            name = f"{initial1.lower()}.{initial2.lower()} {ln}"
        elif style == 1:
            name = f"{initial1.upper()}.{initial2.upper()}. {ln}"
        else:
            name = f"{initial1.upper()} {initial2.upper()} {ln}"
    else:
...
def generate_sentence():
    ...
    # method chunk (ALWAYS with preposition)
    if method:
...
def generate():
    ...
    print("🔥 v4.5 dataset ready (full-name beneficiaries + complex accounts)!")
